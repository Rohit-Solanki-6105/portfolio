...
export default function FloatingCubesScene() {
    return (
        <Canvas
            camera={{ position: [0, 0, 15], fov: 60 }}
            style={{ width: '99vw', height: '100vh', background: '#0d0d0d' }}
        >
    ...
    return (
        <>
            {cubes.map((props, index) => (
                <FloatingCube key={index} {...props} />
            ))}
        </>
    )
}

function FloatingCube({ position, scale, rotationSpeed, floatSpeed, floatOffset }) {
    const ref = useRef()
    ...
    return (
        <mesh ref={ref} position={position} scale={scale}>
            ...
            <meshStandardMaterial
                color="#111111"
                emissive="#0f0f0f"       // glowing color (e.g., indigo)
